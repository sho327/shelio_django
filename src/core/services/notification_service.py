...
class NotificationService:
    """
    アプリケーション全体で発生する通知処理（メール送信、SMSなど）を一括管理するサービス。
    ...
    def _get_site_url(self, path: str) -> str:
        ...
        # settings.DEBUGに応じてスキームを切り替える（元のロジックを維持）
    ...
    def _send_email(
        ...
        try:
            ...
            raise ExternalServiceError(
                ...
                details={"recipient": recipient_list, "internal_error": str(e)},
            )
    ...
    def send_activation_email(self, user_instance: User, raw_token_value: str):
        """
        アクティベーションメールを送信する。
        """
        # 1. URLの構築
        path = reverse("account:activate_user", kwargs={"token_value": raw_token_value})
        activation_url = self._get_site_url(path)

        # 2. メール本文の生成 (元のロジックを維持)
        expiry_seconds = settings.TOKEN_EXPIRY_SECONDS.get("activation", 86400)
        ...
        # 3. 共通メソッドで送信
        self._send_email(subject, message, [user_instance.email])

    def send_password_reset_email(self, user: User, display_name: str, raw_token: str):
        """
        パスワードリセットメールを送信する。
